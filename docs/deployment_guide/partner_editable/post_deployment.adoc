...
==== Test the API Key
Test the API key and `auth` functionality, run a query from the command line to simulate a Salesforce HTTP call to AppSync. You can use the `curl` command or Postman. The following example that you can copy uses the `curl` command.
[source,asciidoc]
....
...
Your results should match the following example, which has been formatted for readability.
[source,asciidoc]
....
{
  "data": {
